 ...
 * 标签初始化
 */
public class PutStoragePlugin extends BasePlugin4Tag {
    private static final String TAG = "PutStoragePlugin";

    @TargetApi(Build.VERSION_CODES.GINGERBREAD_MR1)
    @DoNotRename
    public static ResultBean putStorage(Context context, Tag tag, String key, String EToken, int tagType, int batchId) {
        mContext = context;
        ...
        if (sIsNfcA) {
            ...
            if (tagType == 4) {
                sResultBean = new RasF8023PutStoragePlugin().putStorage(context, tag, key, EToken);
            } else {
                sResultBean = new Ras15693PutStoragePlugin().putStorage(context, tag, key, EToken, tagType, batchId);
            }
        } else {
            sResultBean.setErrno(TagErrorEnum.UN_SUPPORT.getCode());
        }
